...
@section('content')
	<section class="row messages">
		...
		<div class="col-md-6 col-md-offset-3">
			<h2>All Department Public Messages</h2>
			...
			@if(count($department->department_public_messages) > 0)
				@foreach($department->department_public_messages as $department_public_message)
					<article class="message">
						<p>{{$department_public_message->public_message->message->text->text_body}} -- -- {{$department_public_message->public_message->message->text->created_at}}
							<button data-toggle="collapse" data-target="#edit">edit</button>
							<div id="edit" class="collapse">
								<form role="form" method="post" action="/{{$department_public_message->public_message->message->text->text_id}}/edit">
									{{csrf_field()}}
									<input type='text' value="{{$department_public_message->public_message->message->text->text_body}}" name="text_body">
									<input type="submit" value="Done" class="btn btn-success">
								</form>
							</div>
							<form role="form" method="post" action="{{$department_public_message->public_message->message->text->text_id}}/delete">
								{{csrf_field()}}
